...
var _validationTypes = [
  ...
  {value:'natural',text:'natural',regex:'^\\d+$'},
...
function showValidation(control) {	
	...
	if (control) {		
		...
		if (controlClass && controlClass.canValidate) {
			...
			// add the help listener
			addHelp(control.id + "helpValType",true,true,"Choose how to apply the validation for this control. There are various patterns you can choose from to check the control value, or use logic, or even javascript, for more specialised validation. Number allows any positive or negative number including decimal places, currency allow only 2 decimal places, integer does not allow decimals, and natural must be a positive integer");
