...
public abstract class Action {
	...
	protected String getDefaultErrorJavaScript(Application application, Page page, Control control, String offlinePage) {
		...
		String defaultErrorHandler = "alert('Error with " + getErrorSource(application, control) + "\\n\\n' + (server.getResponseHeader('Content-Type') && server.getResponseHeader('Content-Type').startsWith('text/html') && server.responseText.indexOf('<title>' > 0) ? server.status + ' - ' + $(server.responseText).filter('title').text() : server.responseText||message));";
