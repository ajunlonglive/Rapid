...
public abstract class FormAdapter {
	...
	protected String getFormCSV(RapidRequest rapidRequest, String formId) throws Exception {
		...
		sb.append("\"form id\",\"submitted\",\"page id\",\"control id\",\"name\",\"label\",\"value\",\"hidden\"\n");

		// add form id
		sb.append(formId);

		// add submitted date
		sb.append("\"" + rapidRequest.getRapidServlet().getLocalDateTimeFormatter().format(new Date()) + "\"");
			...
			if (pageControlValues != null) {
				...
				for (FormControlValue pageControlValue : pageControlValues) {
					...
					if (control != null) {
						...
						// add nulls for id and submitted if not first line
	...
	protected String getFormXML(RapidRequest rapidRequest, String formId) throws Exception {
		...
		sb.append("<form id=\"" + formId + "\" submitted=\"" + rapidRequest.getRapidServlet().getXMLDateTimeFormatter().format(new Date()) + "\">\n");
		...
		for (String pageId : _application.getPages().getPageIds()) {
			...
			if (pageControlValues != null) {
				...
				// get the page
				Page page = _application.getPages().getPage(pageId);
				...
				// quote enclose and escape if not null
				if (pageName != null) pageName = XML.escape(pageName);
				if (pageLabel != null) pageLabel = XML.escape(pageLabel);

				// open a page collection
				sb.append("\t<page id=\"" + pageId + "\" name=\"" + pageName + "\" label=\"" + pageLabel + "\">\n");
	...
	// return a forms JSON as a string (for attaching or saving to file)
	protected String getFormJSON(RapidRequest rapidRequest, String formId) throws Exception {
		...
		jsonForm.put("submitted", rapidRequest.getRapidServlet().getLocalDateTimeFormatter().format(new Date()));
