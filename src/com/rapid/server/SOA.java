...
public class SOA extends RapidHttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
					...
					if (action == null) {
						...
						if (actionParts.length > 1) {
							...
							if (actionParts.length > 2) {
								// set version if not the literal "null" - supports older versions
								if (!"null".equals(version)) version = actionParts[1];
								// set name
								name = actionParts[2];
							} else {
								// just set name
							...
							// get this application (latest version is not specified)
							application = getApplications().get(appId, version);

							// if the application can't be found
							if (application == null) {
			...
			// if application found check to see if we authenticated
